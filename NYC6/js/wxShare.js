...
$(function($){
    if (!$) 
        return;
    $.fn.wxShare = function(img, width, height, title, desc, url, appid, config){
        _WXShare(img, width, height, title, desc, url, appid, config);
    };
    function _WXShare(img, width, height, title, desc, url, appid, config){
        //初始化参数
        img = img || '';
        width = width || 100;
        height = height || 100;
        ...
        document.addEventListener('WeixinJSBridgeReady', function onBridgeReady(){
            ...
            if (config) {
                wxOptionMenu(config.showOptionMenu !== false);
				wxToolbar(config.showToolbar !== false);
            }
